...
export function RelationshipTimeline() {
  return (
    <div className="py-12">
      <h2 className="text-3xl font-bold text-center text-pink-100 mb-8 font-playfair">Our Love Story</h2>
        ...
        {events.map((event, index) => (
          ...
          >
            <div className={`w-5/12 ${index % 2 === 0 ? 'text-right pr-8' : 'pl-8'}`}>
              <div className="text-sm font-semibold text-pink-200 mb-1">{event.date}</div>
              <h3 className="text-xl font-semibold text-pink-100 mb-2">{event.title}</h3>
      ...
      >
        <ChevronDown className="w-8 h-8 text-pink-100" />
      </motion.div>
    </div>
  )
}
